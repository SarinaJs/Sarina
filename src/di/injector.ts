...
import { Token, ProviderToken } from "./token";
import { generateId } from "./../util";
import { Constructor } from "../../../../../../../../git-source/nest/src/common/utils/merge-with-values.util";
import {
	Provider,
	TypeProvider,
	FactoryProvider,
...
export class Injector {
	...
	private print(level: number = 0) {
		...
		injector.providerWrappers.forEach(wrapper => {
			...
			if ((provider as any).dependencies) {
				(provider as any).dependencies.forEach(dep => {
	...
	constructor(public dependencies: Injector[], providers: Provider[]) {
		providers.forEach(provider => {
			...
			provider.tokens.forEach(pt => {
				...
				if (!tokenWrapper) {
					if (pt instanceof ProviderToken) {
						tokenWrapper = {
							token: pt,
							providerWrappers: []
						};
	...
	public get<T>(token: Token): T {
		let isMulti = false;
		if (token instanceof ProviderToken) {
			isMulti = (token as ProviderToken).multiple;
		}
